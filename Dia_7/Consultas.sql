...
select nombre
from asignatura
where curso=3 and id_grado=7 and cuatrimestre=1;
...
where g.nombre = 'grado en ingeniería informática (plan 2015)' and a.genero = 'f';
...
join matricula m on asig.id = m.id_asignatura
join cursos_escolar ce on m.id_curso_escolar = ce.id
...
select *
from departamento d
where not exists (select 1 from profesor p where p.id_departamento = d.id);

-- 4. Devuelve un listado con los profesores que no imparten ninguna asignatura.
select *
from profesor p
where not exists (select 1 from asignatura a where a.id_profesor = p.id);
...
where not exists (select 1 from matricula m where m.id_asignatura = a.id);
...
select ce.anyo_inicio, count(distinct m.id_alumno) as numero_alumnos
from cursos_escolar ce
